  ...
  STAGES.forEach((stage,idx)=>{
    ...
    container.innerHTML=filtered.map(l=>{
      ...
      return `<div class="lead-card ${l.temp} ${ageClass}" draggable="true" id="${escHtml(l.id)}" data-lead-id="${escHtml(l.id)}" ondragstart="dragStart(event)" onclick="handleCardClick(event,'${escHtml(l.id)}')">
        ...
        <div class="lead-value">${valDisplay}</div><div class="attention-index-row"><span class="ai-label">Attention Index</span><div class="ai-bar-wrap"><div class="ai-bar-fill" style="width:${activityScore}%;background:${aiColor};"></div></div><span class="ai-score" style="color:${aiColor};">${activityScore}</span><span class="ai-tag" style="color:${aiColor};">${aiLabel}</span></div><div class="ai-formula-hint">History×8 + Log links×6 + Next action+10 − Overdue−20</div><div class="ai-alert-hint">${isOverdue?'🔴 Alert active: follow-up date is missed, so card shows a pulse and score is penalized until action is updated.':'🟢 No active alert: this lead has no overdue follow-up right now.'}</div>
        <div>${prodRows}</div>
        <div class="tag-row">
        ...
        <div class="compact-hint">Tap for full details, AI formula notes, and overdue alert context.</div>
      </div>`;
    }).join('');
